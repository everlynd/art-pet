import { Category } from 'src/categories/categories.model';
import { Product } from 'src/product/product.model';

type GenerateId = () => string;
export const generateId: GenerateId = () => (
	Math.random().toString(16).slice(2) + new Date().getTime().toString(36)
);

type GenerateUrl = (url: string) => string;

export const generateUrl: GenerateUrl = (str) => {
	let ru = ("А-а-Б-б-В-в-Ґ-ґ-Г-г-Д-д-Е-е-Ё-ё-Є-є-Ж-ж-З-з-И-и-І-і-Ї-ї-Й-й-К-к-Л-л-М-м-Н-н-О-о-П-п-Р-р-С-с-Т-т-У-у-Ф-ф-Х-х-Ц-ц-Ч-ч-Ш-ш-Щ-щ-Ъ-ъ-Ы-ы-Ь-ь-Э-э-Ю-ю-Я-я").split("-")
	let en = ("A-a-B-b-V-v-G-g-G-g-D-d-E-e-E-e-E-e-ZH-zh-Z-z-I-i-I-i-I-i-J-j-K-k-L-l-M-m-N-n-O-o-P-p-R-r-S-s-T-t-U-u-F-f-H-h-TS-ts-CH-ch-SH-sh-SCH-sch-'-'-Y-y-'-'-E-e-YU-yu-YA-ya").split("-")
	let res = '';
	for (let i = 0, l = str.length; i < l; i++) {
		let s = str.charAt(i), n = ru.indexOf(s);
		if (n >= 0) { res += en[n]; }
		else { res += s; }
	}
	res = res.replace(/\ ? /g, '-') + "-" + Math.random().toFixed(3).slice(2).replace(/\./g, '')
	return res;
}


export const appendChildrens = (array: Category[], products: Product[]) => {
	let result = [];
	for (let i = 0; i < array.length; i++) {
		result.push(array[i])
	}
	for (let i = 0; i < array.length; i++) {
		if (array[i].parentId) {
			products.forEach(item => {
				if (item.categoryId === array[i].id) {
					array[i].itemsCount += 1

				}
			})
			result.find(elem => elem.id === array[i].parentId)?.children?.push(array[i])
		}
	}
	return result;
}